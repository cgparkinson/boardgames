...
class TestChess(unittest.TestCase):
    ...
    def test_pawn_move_2(self):
        Turn(actions=[Move((0,1), (0,3))], player=self.chess.black).perform(board_game_state=self.chess.state)
    ...
    def test_pawn_move_3(self):
        Turn(actions=[Move((0,1), (0,4))], player=self.chess.black).perform(board_game_state=self.chess.state)
        assert isinstance(self.board.get_item(location=(0,3)), Pawn)
